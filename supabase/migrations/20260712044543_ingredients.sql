-- Ingredient dictionary schema. ALREADY APPLIED to the production project as
-- remote migration version 20260712044543 (2026-07-12) — this file records the
-- DDL locally under the same version so `supabase migration list` stays in
-- sync and `db push` never re-applies it. Reuses the post_status enum and
-- set_updated_at() trigger from 0001. Public reads published rows; writes
-- require an authenticated session (app-layer admin allowlist).
